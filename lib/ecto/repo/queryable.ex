defmodule Ecto.Repo.Queryable do
  ...
  defp preprocess(prefix, sources, adapter) do
    &preprocess(&1, &2, prefix, sources, &3, adapter)
  end

  defp preprocess({:&, _, [ix]}, value, prefix, sources, context, adapter) do
    {source, model} = elem(sources, ix)
    Ecto.Schema.__load__(model, prefix, source, context, value, &adapter.load/2)
  end

  defp preprocess({{:., _, [{:&, _, [_]}, _]}, meta, []}, value, _prefix, _sources, _context, adapter) do
  ...
  defp preprocess(%Ecto.Query.Tagged{tag: tag}, value, _prefix, _sources, _context, adapter) do
    load!(tag, value, adapter)
  end

  defp preprocess(_key, value, _prefix, _sources, _context, _adapter) do
    value
  end
